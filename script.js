...
async function handleSolicitudSubmit(e) {
    ...
    try {
        ...
        // --- Notificación Telegram ---
        const machine = state.machines.find(m => m.id === machineId);
        const typeTitle = type === 'mecanizado' ? 'Mecanizado' : 'Mantenimiento';
...
async function saveWorkOrder(updates = {}) {
    ...
    try {
        ...
        if (isNew) {
            ...
            // Notificación Telegram para nuevas OTs (Manuales)
            // No notificamos si ya viene de una Solicitud (porque esa ya notificó al crearse)
            if (!sourceSolicitudId && orderData.status !== 'Cancelado') {
                ...
                const tgMsg = `🛠 <b>Nueva Orden de Trabajo</b>\n\n` +
                              ...
                              `<b>Técnico:</b> ${escapeHTML(orderData.leadTechnician)}\n` +
...
async function showPrettyExecutionReport(executionId) {
    const execution = state.workPlanExecutions.find(e => e.fb_id === executionId);
    if (!execution) {
        showToast('No se encontró la ejecución solicitada.', 'error');
        return;
    }

    const plan = state.workPlans.find(p => p.fb_id === execution.planId);
    const machine = state.machines.find(m => m.id === plan?.machineId);
    const container = document.getElementById('pretty-report-content');

    const dateStr = new Date(execution.executedAt).toLocaleDateString('es-ES', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric',
    ...
    let html = `
        <div class="p-5">
            <div class="text-center mb-5">
                <div class="badge bg-primary-soft text-primary px-3 py-2 rounded-pill mb-3">Reporte de Mantenimiento</div>
                <h1 class="fw-800 display-5 mb-2">${plan?.name || 'Ejecución de Plan'}</h1>
                <p class="text-muted lead">Finalizado el ${dateStr}</p>
            </div>

            <div class="row g-4 mb-5">
                <div class="col-md-4">
                    <div class="card border-0 bg-light p-4 h-100" style="border-radius: 15px;">
                        <span class="text-muted x-small text-uppercase fw-bold mb-2">Máquina Intervenida</span>
                        <h5 class="fw-bold mb-0 text-primary">${machine?.name || plan?.machineId || 'N/A'}</h5>
                        <small class="text-muted mt-1">${machine?.id || ''}</small>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card border-0 bg-light p-4 h-100" style="border-radius: 15px;">
                        <span class="text-muted x-small text-uppercase fw-bold mb-2">Técnico Responsable</span>
                        <h5 class="fw-bold mb-0 text-dark">${execution.executedBy || 'N/A'}</h5>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="card border-0 bg-light p-4 h-100" style="border-radius: 15px;">
                        <span class="text-muted x-small text-uppercase fw-bold mb-2">Estado General</span>
                        <h5 class="fw-bold mb-0 text-success"><i class="fas fa-check-circle me-1"></i> Completado</h5>
                    </div>
                </div>
            </div>

            <div class="mb-5">
                <h5 class="fw-bold border-bottom pb-2 mb-4">Resumen de Tareas</h5>
                <div class="row g-4">
    `;
    ...
    (execution.taskGroups || []).forEach(group => {
                ...
                <div class="card border-0 shadow-sm overflow-hidden" style="border-radius: 15px;">
                    ...
                    <div class="card-body p-0">
                        <table class="table table-hover mb-0">
                            <thead class="bg-light">
                                <tr>
                                    <th class="ps-4" style="width: 50%;">Tarea</th>
                                    <th class="text-center">Resultado</th>
                                    <th class="pe-4">Detalles / Medición</th>
                                </tr>
                            </thead>
                            <tbody>
        `;
        ...
        group.tasks.forEach(task => {
            let statusBadge = '';
            if (task.status === 'Aprobó') statusBadge = '<span class="badge bg-success rounded-pill px-3">Aprobó</span>';
            else if (task.status === 'Fallo') statusBadge = '<span class="badge bg-danger rounded-pill px-3">Fallo</span>';
            else if (task.status === 'Alerta') statusBadge = '<span class="badge bg-warning text-dark rounded-pill px-3">Alerta</span>';
            else statusBadge = '<span class="badge bg-secondary rounded-pill px-3">Pendiente</span>';

            let valueDisplay = '-';
            if (task.type === 'numeric' && task.value) valueDisplay = `<strong>${task.value} ${task.unit || ''}</strong>`;
            else if (task.type === 'multi_numeric' && task.multiValues) {
                valueDisplay = task.fields.map((f, i) => `${f}: <strong>${task.multiValues[i] || '0'}</strong>`).join(' | ');
            }

            if (task.detail) {
                valueDisplay += `<div class="x-small text-muted mt-1">${task.detail}</div>`;
            }
            ...
            html += `
                <tr>
                    ...
                    <td class="pe-4 py-3">${valueDisplay}</td>
                </tr>
            `;
        });
