    ...
    <main class="pdf-page">
        ...
        <header class="border-b-2 border-primary pb-3 mb-4 flex justify-between items-start">
            <div class="flex gap-4 items-center">
                ...
                <div>
                    ...
                    <p class="text-[10px] text-slate-400">Doc ID: CMMS-FA-001 | Versión 2.0</p>
                </div>
            </div>
        ...
        <footer class="mt-auto border-t border-slate-200 pt-4 flex justify-between items-end">
            ...
            <div class="flex flex-col items-center gap-1">
                ...
                <span class="text-[8px] text-slate-400 font-bold tracking-widest">VALIDACIÓN QR</span>
            </div>
        </footer>
    </main>
</body>
...
function updateTechnicianModalUIForRole(role) {
    ...
    const showTecnicoMaquinas = role === 'Técnico' || role === 'Operario';
...
async function handleTechnicianSubmit(e) {
    ...
    if (techId && role !== 'Técnico' && role !== 'Operario') {
...
async function handleSolicitudSubmit(e) {
    ...
    try {
        ...
        // --- Notificación Telegram ---
        const machine = state.machines.find(m => m.id === machineId);
        const msg = `🆕 *Nueva Solicitud de Mantenimiento*\n\n` +
                    `*ID:* ${nextId}\n` +
                    `*Equipo:* ${machine ? machine.name : machineId} (${machineId})\n` +
                    `*Solicitante:* ${solicitudData.requester}\n` +
                    `*Descripción:* ${solicitudData.description}\n` +
    ...
    row.innerHTML = `
        ...
        <td>
            <div class="status-btn-group">
                <button class="btn-status ok ${task.status === 'Aprobó' ? 'active' : ''}" data-status="Aprobó">OK</button>
...
async function handleKanbanWorkOrderAction(workOrderFbId, newStatus) {
    ...
    try {
        ...
        if (finalStatus === 'Pendiente de Evaluación' || finalStatus === 'Completado' || finalStatus === 'Pendiente de Aprobación') {
            ...
            // Si ya estaba en un estado que descuenta stock, pasamos el objeto original para calcular el delta (que será 0 si no cambiaron repuestos)
...
async function syncPartStockToOdoo(partId, partData = null) {
    ...
    try {
        ...
        if (!odooLocationId) {
            console.warn(`[Odoo Sync] No se encontró una ubicación interna válida en Odoo para sincronizar stock de ${partId}`);
            return;
        }
...
async function generateMachineReport() {
    ...
    try {
        ...
        let mtbf = 'N/A';
        // MTBF actualizado: (Tiempo total desde creación hasta hoy) / (Fallas + 1)
...
function renderSmartMonitoring() {
    ...
    machinesToRender.forEach(machine => {
        ...
        if (overallStatus !== 'operativo' && overallStatus !== lastStatus) {
            ...
            let alertMsg = `⚠️ *ALERTA DE MONITOREO: ${machine.name}*\n\n` +
            ...
            failingVars.forEach(v => {
                alertMsg += `• ${v.key}: ${v.value}${v.unit || ''} (${v.status})\n`;
            });
